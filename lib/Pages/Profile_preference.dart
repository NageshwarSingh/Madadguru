...
class _ProfilePreferenceState extends State<ProfilePreference> {
     ...
     Future<void> ProfileSelectedAPI(String value) async {
      ...
      try {
        final response = await http.post(
          ...
          print('Failed to send need_help_in selection to API. Status code: ${response.statusCode}');
        }}catch (error) {
  ...
  Future<void> fetchData() async {
    ...
    var usertoken = prefs.getString('token');
    if (usertoken != null) {
      final Uri uri = Uri.parse("https://madadguru.webkype.net/api/getNeedHelp");
      try {
        final response = await http.post(
          uri,
          headers: {
          ...
          SingleChildScrollView(
            child: Column(
              ...
              ChipsChoice<dynamic>.multiple(
                ...
                value: department.map((item) => item['id'].toString()).toList(),
                onChanged: (List<dynamic> val) {
                  setState(() {

                    department = DepartmentList.where((item) => val.contains(item['id'].toString())).toList();
                    needhelpin = department.map((item) => item['id'].toString()).join(',');
                ...
                choiceStyle: C2ChipStyle.outlined(
                  ...
                  foregroundStyle:
                  const TextStyle(color: Color(0xFF2F2924), fontSize: 12),
                  selectedStyle: C2ChipStyle.filled(
                    foregroundStyle: TextStyle(color: Colors.white, fontSize: 12),
